...
namespace testing.Controllers
{
    ...
    public class AccountController : Controller
    {
        public AccountController()
            : this(new UserManager<ApplicationUser, int>(new UserStore<ApplicationUser, MyRole, int, MyLogin, MyUSerRole, MyClaim>(new ApplicationDbContext())))
        {
        }
        ...
        public async Task<ActionResult> Login(User_Account  model, string ReturnUrl)
        {
            if (ModelState.IsValid)
            {
                var user = await UserManager.FindAsync(model.UserName, model.PasswordHash);
                if (user != null)
                {
                    await SignInAsync(user, model.IsConfirmed );
